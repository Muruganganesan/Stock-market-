...
# --- Load Data Safely ---
if not os.path.exists("merged_and_sorted.csv") or not os.path.exists("Sector_data.csv"):
    st.error("Required CSV files not found. Please ensure 'merged_and_sorted.csv' and 'Sector_data.csv' are available.")
    st.stop()
...
# --- 6. Monthly Top 5 Gainers & Losers ---
    ...
    month_selected = st.selectbox("Select Month", sorted(unique_months), index=len(unique_months)-1)
